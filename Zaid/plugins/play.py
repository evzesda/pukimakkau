...
btnn =[
    [Button.url("οωиєя ️", url=f"t.me/{Config.SUPPORT}"), Button.url("gяουρ ", url=f"t.me/{Config.CHANNEL}")],
    [Button.inline("ϲℓοѕє ", data="cls")]]
...
async def play(event):
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**gινє мє γουя գυєяγ ωнιϲн γου ωαиτ το ρℓαγ**\n\n **єϰαмρℓє**: `/play baby angel`", buttons=btnn)
    elif replied and not replied.audio and not replied.voice or not replied:
        botman = await event.reply("**🔄 ρяοϲєѕѕιиg գυєяγ... ρℓєαѕє ωαιτ!**")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        if search == 0:
            await botman.edit(
                "**ϲαи'τ ƒιи∂ ѕοиg** τяγ ѕєαяϲнιиg ωιτн мοяє ѕρєϲιƒιϲ τιτℓє"
            )     
        else:
            songname = search[0]
            title = search[0]
...
async def vplay(event):
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**gινє мє γουя գυєяγ ωнιϲн γου ωαиτ το ρℓαγ**\n\n **єϰαмρℓє**: `/vplay baby angel`", buttons=btnn)
    if replied and not replied.video and not replied.document:
        xnxx = await event.reply("**🔄 ρяοϲєѕѕιиg գυєяγ... ρℓєαѕє ωαιτ!**")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        RESOLUSI = 720
            ...
            if hm == 0:
                ...
                try:
                    ...
                    await event.client.send_file(event.chat_id,
                        f"**📡 ѕταяτє∂ ѕτяєαмιиg 💡**\n\n💡 **τιτℓє:** [{songname}]({url})\n**⏰ ∂υяατιοи:** `{duration}`\n👥 **яєգυєѕτє∂ ϐγ:** {from_user}, buttons=btnn",
                        link_preview=False,
                    )
                except Exception as ep:
                    clear_queue(chat_id)
